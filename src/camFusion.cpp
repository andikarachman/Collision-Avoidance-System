...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    // calculate average distance of all matched keypoints inside the bounding box
    float totDist = 0.0f;
    int kptSize = 0;
    for (cv::DMatch kptMatch : kptMatches)
    {
        auto& currPt = kptsCurr[kptMatch.trainIdx].pt;
        if (boundingBox.roi.contains(currPt))
        {
            totDist = totDist + kptMatch.distance;
            kptSize++;
        }
    } 
    ...
    if (kptSize != 0) 
    {
        float avgDist = totDist / kptSize;
        for (cv::DMatch kptMatch : kptMatches)
        {
            auto& currPt = kptsCurr[kptMatch.trainIdx].pt;
            if (boundingBox.roi.contains(currPt) && kptMatch.distance < avgDist) 
            {
...
{
    std::sort(lidarPoints.begin(), lidarPoints.end(), [](LidarPoint ptA, LidarPoint ptB) 
    {
        return ptA.x < ptB.x; 
    }
    );
...
{
    // Take the median of lidar points in x-direction as estimate.
                        ...
                        DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    cv::Mat countTable = cv::Mat::zeros(prevBbSize, currBbSize, CV_32S);

    for (cv::DMatch& match : matches)
    {
        const auto& prevPt = prevFrame.keypoints[match.queryIdx].pt;
        const auto& currPt = currFrame.keypoints[match.trainIdx].pt;

        for (int i = 0; i < prevBbSize; ++i) 
        {
            const auto& prevFrameROI = prevFrame.boundingBoxes[i].roi;
            for (int j = 0; j < currBbSize; ++j) 
            {
                const auto& currFrameROI = currFrame.boundingBoxes[j].roi;
                if (prevFrameROI.contains(prevPt) && currFrameROI.contains(currPt)) 
                {
                    countTable.at<int>(i, j)++;
                }
            }
        }
    }

    int bestMatchValue, bestMatchIdx;
    for (int i = 0; i < countTable.rows; ++i) 
    {
        bestMatchIdx = -1;
        bestMatchValue = 0;
        for (int j = 0; j < countTable.cols; ++j) 
        {
            if (countTable.at<int>(i, j) > bestMatchValue) 
            {
                bestMatchIdx = j;
                bestMatchValue = countTable.at<int>(i, j);
            }           
        }
        if (bestMatchIdx != -1) 
        {
